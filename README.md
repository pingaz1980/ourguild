...
3. 个人赊分上限为700分，副本活动超过2CD之后，上限每CD降低70分，公式为：MIN[ 700, 840 - 副本CD数 x 70 ]
4. 赊分资格：至少完整参加过两周的公会活动，第三周可以赊分300分，第四周可以赊分到上限
5. 每周活动结束后根据实际DKP计算抵押金币余额，多余部分退回个人 

注1：赊分制度的目的有三个：一、避免初期极品出分过低；二、避免中途加入的新人无分可用；三、避免DKP快速膨胀

### 奖惩

1. 非开荒期间的战斗中出现低级错误的，一次扣除5分DKP，导致灭团的扣除10分DKP
2. 月度全勤的所有成员，每人奖励10分DKP
3. 未请假迟到1小时以内的扣除DKP2分，未请假迟到超过1小时的扣除DKP5分，未请假迟到超过3小时的按未到处理。
4. 未请假未参加活动的扣除DKP10分，且下一个CD不得主动出分拿装备；连续未请假未参加活动两次的，第二次扣除DKP20分，且下两个CD不得主动出分拿装备；连续未请假未参加活动三次的，第三次扣除DKP100分，且从名单去除，等恢复活动的时候前三个CD不得拿装备。
...
橙杖每次按一套进行分配，分配原则是参照历史DKP的总和与职业天赋。具体说明如下：

1. 历史DKP的总和是指BWL、TAQ与NAXX三个副本历史DKP的总和，总和第一名分数的95%（含）以上为第一档，90%（含）以上为第二档，80%（含）以上为第三档，70%（含）以上为第四档，50%（含）以上为第五档，其他为第六档。
2. 从第一档开始计算分配资格，若同档可分配人有多人时，则采取roll 1-历史DKP总和的方式确定获得者。
3. 获得者的DKP扣除NAXX的12个CD可获取的总DKP分数，即840分。
4. 获得者同时需缴纳抵押金（数额为10000金+负分抵押金），等到所有活动等级的DKP为正分时再归还。
5. 获得橙杖分配资格的，若当月出勤率低于50%的，不得获取剩余碎片，需重新计算分配资格。
...
| 物品           | 起拍分 | 特殊规则                                                     |
| -------------- | ------ | ------------------------------------------------------------ |
| 克尔苏加德武器 | 300    | CL时间：开荒以及FARM的前四周<br />CL范围：进入过Boss DPS奖励榜单 |
| 克尔苏加德散件 | 100    | 如果无人出分，由所有装备属性低于该物品的相关职业天赋勇士Roll点所得，并扣取起拍分 |
| 武器和饰品     | 100    | 如果无人出分，由所有装备属性低于该物品的相关职业天赋勇士Roll点所得，并扣取起拍分 |
| 其他散件       | 50     | 如果无人出分，由所有装备属性低于该物品的相关职业天赋勇士Roll点所得，并扣取起拍分 |
| 垃圾散件       | 10     | 如果无人出分，由所有未拥有过该物品以及装备属性低于该物品的的相关职业天赋的勇士Roll点所得，并扣取起拍分 |
| 职业套装       | 50     | 封顶为起拍分三倍，达到时由各出分人按自己剩余DKP来Roll点。<br />比如出分人A剩余DKP为200分，这打/roll 200，roll点高的获得该装备。若无人出分，则有从未拥有过该物品的的相关职业天赋的勇士Roll点所得，并扣取起拍分 |
